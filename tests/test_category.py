...
def test_category_products_property(category1_fixture: Any):
    assert category1_fixture.products == ('Samsung Galaxy C23 Ultra, 180000.0 руб. Остаток: 5 шт.\n'
                                          'Iphone 15, 210000.0 руб. Остаток: 8 шт.\n')


def test_category_products_setter(category1_fixture: Any, product_fixture):
    assert len(category1_fixture.list_of_products) == 2
    category1_fixture.products = product_fixture
    assert len(category1_fixture.list_of_products) == 3


def test_category_add_product(category1_fixture: Any, product_fixture):
